...
namespace OscarBrouwer.Framework.Testing {
  ...
  public class TestRepository<T> : Repository<T>, IDisposable where T : class, IEntity<T>, new() {
    ...
    protected override T AddEntityCore(T entity) {
      try {
    ...
    protected override T CreateEntityCore() {
      return new T();
    }
    ...
    protected override void DeleteEntityCore(T entity) {
      try {
    ...
    protected override IEnumerable<T> FindAllCore(Func<T, bool> expression) {
      try {
    ...
    protected override IEnumerable<T> FindAllCore() {
      try {
        this.temporaryStorageLock.EnterReadLock();
        storageLock.EnterReadLock();
    ...
    protected override T FindFirstCore(Func<T, bool> expression, T defaultValue) {
      try {
        this.temporaryStorageLock.EnterReadLock();
    ...
    protected override T FindFirstCore(Func<T, bool> expression) {
      try {
        this.temporaryStorageLock.EnterReadLock();
        T result = this.temporaryStorage.FirstOrDefault(item => expression(item.Key)).Key;
        this.temporaryStorageLock.ExitReadLock();
    ...
    protected override T FindSingleCore(Func<T, bool> expression, T defaultValue) {
      try {
        this.temporaryStorageLock.EnterReadLock();
    ...
    protected override T FindSingleCore(Func<T, bool> expression) {
      try {
        this.temporaryStorageLock.EnterReadLock();
        T result = this.temporaryStorage.SingleOrDefault(item => expression(item.Key)).Key;
        this.temporaryStorageLock.ExitReadLock();
    ...
    protected override T UpdateEntityCore(T entity) {
      try {
        /* First search the temporary storage */
        this.temporaryStorageLock.EnterWriteLock();
        if(this.temporaryStorage.Any(kvp => kvp.Key.RecordId == entity.RecordId)) {
