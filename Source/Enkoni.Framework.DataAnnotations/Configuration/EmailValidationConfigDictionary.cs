...
namespace Enkoni.Framework.DataAnnotations.Configuration {
  /// <summary>Defines a custom dictionary that holds the <see cref="EmailValidationConfigElement"/> that were read from the configuration
  /// file.</summary>
  public class EmailValidationConfigDictionary : IDictionary<string, EmailValidationConfigElement> {
    #region Instance variables

    /// <summary>The actual storage.</summary>
    private Dictionary<string, EmailValidationConfigElement> storage;

    #endregion

    #region Constructor

    /// <summary>Initializes a new instance of the <see cref="EmailValidationConfigDictionary"/> class.</summary>
    public EmailValidationConfigDictionary() {
      this.storage = new Dictionary<string, EmailValidationConfigElement>();
    }
    ...
    public ICollection<EmailValidationConfigElement> Values {
    ...
    public EmailValidationConfigElement this[string key] {
      get {
        if(this.storage.ContainsKey(key)) {
          return this.storage[key];
        }
        else if(this.storage.ContainsKey(EmailAttribute.DefaultName)) {
          return this.storage[EmailAttribute.DefaultName];
        }
        else {
          return null;
        }
    ...
    public void Add(string key, EmailValidationConfigElement value) {
      this.storage.Add(key, value);
    }

    /// <summary>Adds an item to the <see cref="ICollection{T}"/>.</summary>
    /// <param name="item">The object to add to the <see cref="ICollection{T}"/>.</param>
    public void Add(KeyValuePair<string, EmailValidationConfigElement> item) {
      ((IDictionary<string, EmailValidationConfigElement>)this.storage).Add(item);
    }
    ...
    public bool Contains(KeyValuePair<string, EmailValidationConfigElement> item) {
    ...
    public bool Remove(KeyValuePair<string, EmailValidationConfigElement> item) {
      return ((IDictionary<string, EmailValidationConfigElement>)this.storage).Remove(item);
    }
    ...
    /// <exception cref="ArgumentNullException"><paramref name="key"/> is <see langword="null"/>.</exception>
    public bool TryGetValue(string key, out EmailValidationConfigElement value) {
    ...
    public void CopyTo(KeyValuePair<string, EmailValidationConfigElement>[] array, int arrayIndex) {
      ((IDictionary<string, EmailValidationConfigElement>)this.storage).CopyTo(array, arrayIndex);
    }

    /// <summary>Returns an enumerator that iterates through the collection.</summary>
    /// <returns>A <see cref="IEnumerator{T}"/> that can be used to iterate through the collection.</returns>
    public IEnumerator<KeyValuePair<string, EmailValidationConfigElement>> GetEnumerator() {
