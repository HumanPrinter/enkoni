...
namespace OscarBrouwer.Framework {
  ...
  public interface ISpecificationVisitor<T> {
    ...
    Func<T, bool> CreateLambdaExpression(Func<T, bool> expression);
    ...
    /// </param>
    /// <returns>The created expression.</returns>
    Func<T, bool> CreateLikeExpression(Func<T, string> field, string pattern);
    ...
    /// <returns>The created expression.</returns>
    Func<T, bool> CreateAndExpression(ISpecification<T> leftOperand, ISpecification<T> rightOperand);

    /// <summary>Creates an OR-expression using the two specified specifications.</summary>
    /// <param name="leftOperand">The left operand of the combination.</param>
    /// <param name="rightOperand">The right operand of the combination.</param>
    /// <returns>The created expression.</returns>
    Func<T, bool> CreateOrExpression(ISpecification<T> leftOperand, ISpecification<T> rightOperand);
    ...
    /// <returns>The created expression.</returns>
    Func<T, bool> CreateCustomExpression(ISpecification<T> specification);
  }
}
