...
Pack-NuGet -releaseInfo $releaseInfo -outputDirectory .\WorkingDir
$exitCode = $LastExitCode
if($exitCode -ne 0) {
  return
}
...
msbuild /p:Configuration=Release /p:DumpLogOnFailure=True .\Source\Enkoni.Documentation.shfbproj
$exitCode = $LastExitCode
if($exitCode -ne 0) {
...
Write-Host "  - Build solution:`t`t`t`t`t"($buildTime - $readNotesTime) -ForegroundColor Green
Write-Host "  - Execute unit tests:`t`t`t`t"($testTime - $buildTime) -ForegroundColor Green
...
Write-Host "  - NuGet package creation:`t`t`t"($nugetTime - $signTime) -ForegroundColor Green
Write-Host "  - Build documentation:`t`t`t"($docuTime - $nugetTime) -ForegroundColor Green
